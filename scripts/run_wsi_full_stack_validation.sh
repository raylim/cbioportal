...
BACKEND_DIR="${BACKEND_DIR:?BACKEND_DIR must point at the pinned backend foundation checkout}"
BACKEND_FOUNDATION_REF="${BACKEND_FOUNDATION_REF:?BACKEND_FOUNDATION_REF is required}"
...
FRONTEND_REF="${FRONTEND_REF:?FRONTEND_REF must match the selected frontend variant}"
...
WSI_VARIANT="${WSI_VARIANT:-combined}"

test -x "$TEST_STACK_DIR/utils/check-connection.sh"
test "$(git -C "$BACKEND_DIR" rev-parse HEAD)" = "$BACKEND_FOUNDATION_REF"
test "$(git -C "$FRONTEND_DIR" rev-parse HEAD)" = "$FRONTEND_REF"
...
build_portal_image() {
  log "building portal image ${IMAGE} from backend $(git -C "$BACKEND_DIR" rev-parse HEAD) with Core ${CORE_REF}"
  docker build \
    --build-arg CORE_REPOSITORY="$CORE_REPOSITORY" \
    --build-arg CORE_REF="$CORE_REF" \
    --tag "$IMAGE" \
    --file "$BACKEND_DIR/docker/web-and-data/Dockerfile" \
    "$BACKEND_DIR"
  docker image inspect "$IMAGE" --format 'portal image={{.Id}} created={{.Created}}' | tee -a "$FULL_STACK_LOG"
}
...
CBIOPORTAL_BACKEND_SOURCE_DIR=${BACKEND_DIR}
...
import_fixture_and_check_lifecycle() {
  ...
  local slide_count
  local study_db_id
  study_db_id="$(docker exec cbioportal-database-container sh -lc '
    clickhouse-client --user "$CLICKHOUSE_USER" --password "$CLICKHOUSE_PASSWORD" --database "$CLICKHOUSE_DB" --query "SELECT cancer_study_id FROM cancer_study WHERE cancer_study_identifier = '\''msk_spectrum_tme_2022'\'' FORMAT TSVRaw"
  ')"
  test -n "$study_db_id"
  ...
  test "$slide_count" = 4
  local wsi_table
  local wsi_tables=(wsi_patient wsi_part wsi_block wsi_slide wsi_slide_placement wsi_slide_timing)
  for wsi_table in "${wsi_tables[@]}"; do
    local imported_rows
    imported_rows="$(docker exec cbioportal-database-container sh -lc "
      clickhouse-client --user \"\$CLICKHOUSE_USER\" --password \"\$CLICKHOUSE_PASSWORD\" --database \"\$CLICKHOUSE_DB\" --query \"SELECT count() FROM ${wsi_table} WHERE cancer_study_id = ${study_db_id}\"
    ")"
    test "$imported_rows" -gt 0
  done
...
run_browser_tests() {
  ...
  for browser_test in "${browser_tests[@]}"; do
    if [[ ! -f "$browser_dir/$browser_test" ]]; then
      echo "required WSI browser test is missing: $browser_test" >&2
      exit 1
    fi
  done
  (cd "$browser_dir" && \
